        ...
        let request =
            if let Some(chunk_requester) = body {
                ...
                runtime.spawn({
                    ...
                    .to_stream()
                    .fold(
                        (sink, fetch_terminated, chunk_requester.clone(), false),
                        |(mut sink, fetch_terminated, chunk_requester, done), message| async move {
                            let mut closed = done;
                            match message.unwrap() {
                                ...
                                BodyChunkResponse::Error if !done => {
                                    // Step 4 and/or 5.
                            ...
                            (sink, fetch_terminated, chunk_requester, closed)
                        },
                    )
                    .map(|_| ())
                });
